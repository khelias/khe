...
Required fields:
  .claude/agents/*.md             name (str), description (str), tools (list[str]), model (str)
  .claude/skills/<name>/SKILL.md  name (str, equal to <name>), description (str)

README.md inside those directories is excluded (those are navigation files,
not agents or skills). A flat .claude/skills/*.md file is an error: Claude Code only
...
def candidate_files(directory: str) -> tuple[list[Path], list[str]]:
    ...
    errors = [
        ...
        f"move it to .claude/skills/{md.stem}/SKILL.md"
        for md in flat
    ]
